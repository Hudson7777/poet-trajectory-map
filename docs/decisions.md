...
- **D10 朝代扩展性**：朝代一等公民。`data/dynasties.yaml` 注册表（id/name/era/divisionName/basemap/cities）；地理数据按朝代分桶 `data/geo/<dynasty>/`；人物数据按朝代分桶 `data/poets/<dynasty>/`；作品有 `genre` 字段（诗/词/文/赋/曲）；生平节点公元年份为键 + 可选年号 eraName 仅展示；扩新朝代 = 新增地理文件 + 人物文件 + 注册表一行，代码零改动。
